...
export class EnrollementService {
    ...
    try {
      const result = await this.em.transactional(async (em) => {
        ...
        const course = await em.findOne(
          ...
          { populate: ['professor', 'professor.user'] }
        );
        if (!course) {
        ...
        if (
          ...
          user.professorProfile.id === (course.professor as Professor).id
        ) {
          this.logger.warn(
            { userId: studentId, courseId },
            'Professor attempted to enroll in their own course.'
          );
          throw new Error(
            'Un profesor no puede inscribirse en su propio curso.'
          );
        }

        const already = await em.findOne(Enrollement, {
          student: student.id,
          course: course.id,
        });
        if (already) {
          this.logger.warn(
            ...
            'Enrollment already exists, returning existing one.'
          );
          return already;
        }
        ...
        if (course.isFree) {
          ...
          const professorName = professorUser
            ? `${professorUser.name} ${professorUser.surname}`
            : 'Profesor';
          const frontendUrl =
      ...
      this.logger.info(
        { id: result.id, studentId: result.student.id, courseId },
   ...
   * @param {string} studentId - The ID of the Student or User.
   * @param {string} courseId - The ID of the Course.
   * @returns {Promise<EnrollmentDetailResponse | null>} The filtered enrollment or null if not found.
   */
  async findByStudentAndCourse(
    studentId: string,
    courseId: string
  ): Promise<EnrollmentDetailResponse | null> {
    let enrollment: Enrollement | null = null;
    ...
    try {
      ...
      if (user && user.studentProfile) {
        ...
        enrollment = await this.em.findOne(
          Enrollement,
          {
          ...
          },
          {
            populate: [
              ...
              'course.courseType',
              'course.institution',
              'course.units.materials',
              'course.units.questions',
            ],
          }
        );
      }
    ...
    return mapEnrollmentToDetail(enrollment);
  }
   ...
   * @returns {Promise<EnrollmentListResponse[]>} An array of filtered enrollment responses.
   */
  async findByStudent(studentId: string): Promise<EnrollmentListResponse[]> {
    ...
    const enrollmentsWithCounts = await Promise.all(
      enrollments.map(async (enrollment) => {
        ...
        return mapEnrollmentToList(enrollment, studentsCount);
      })
    );
  ...
  async update(
    ...
    const enrol = await this.em.findOneOrFail(Enrollement, {
      _id: new ObjectId(id),
    });
